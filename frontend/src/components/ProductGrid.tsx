...
export default function ProductGrid({
  ...
  function matchesSparkling(product: Product) {
    ...
    return ['sparkling', 'champagne', 'prosecco', 'cava', 'brut'].some(
      (term) => subType.includes(term)
    );
  }
  ...
  const visibleProducts =
    filter === 'all' ? featuredProducts : filteredProducts;
  ...
  return (
    <section className="bg-stone-50 py-12 md:py-20" id="products">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* CATEGORY */}
        <div
          className="-mt-16 rounded-[2rem] border border-white/80 bg-white/90 px-4 py-8 shadow-2xl shadow-stone-950/8 backdrop-blur-xl md:-mt-24 md:px-8 md:py-12"
          id="wine-categories"
        >
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {filters.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br ${item.accent}`}
                  >
                    <div
                      className={`absolute inset-4 rounded-full ${item.glow}`}
                    />
                  ...
                  <span
                    className={`mt-3 text-sm font-extrabold uppercase tracking-[0.14em] ${
                      isActive
                        ? 'text-white'
                      ...
                      <div className="mt-auto space-y-4">
                        <div className="flex items-center justify-between border-t border-stone-100 pt-4">
                          ...
                          <button
                            ...
                            className="min-h-12 rounded-full bg-stone-950 px-5 py-3 text-xs font-extrabold uppercase tracking-widest text-white transition-all hover:bg-primary disabled:cursor-not-allowed disabled:bg-stone-400"
                          >
