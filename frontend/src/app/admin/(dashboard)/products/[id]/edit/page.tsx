...
import { Wine, Save, ArrowLeft, Loader2, AlertTriangle } from 'lucide-react';
...
export default function AdminEditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        setStock(json.product.stock.toString());
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('ไม่สามารถแก้ไขข้อมูลสินค้าได้');
      }
  ...
  if (loading) {
    return (
      <div className="space-y-6 max-w-2xl select-none font-sans mx-auto animate-pulse">
        <div className="h-6 w-32 bg-stone-900 rounded" />
        <div className="h-24 w-full bg-stone-900 rounded-2xl" />
        <div className="h-96 w-full bg-stone-900 rounded-2xl" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl select-none font-sans mx-auto">
      ...
      <div className="bg-stone-900 border border-white/5 rounded-2xl p-6 flex items-center gap-4">
        ...
        <div>
          <h2 className="text-lg font-bold font-serif text-stone-100">แก้ไขรายละเอียดสินค้า #{productId}</h2>
          <p className="text-xs text-stone-400 mt-0.5">แก้ไขข้อมูลและจัดเก็บการตั้งค่าของไวน์</p>
        </div>
      </div>
