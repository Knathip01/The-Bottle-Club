...
exports.getCountryLabel = exports.DEMO_TRACKING_IDS = exports.EXPORT_DESTINATION_COUNTRIES = exports.DELIVERY_ROUTE = exports.TH_LOCAL_ROUTE = void 0;
exports.resolveShipment = resolveShipment;
exports.getDestinationLabel = getDestinationLabel;
exports.getModeLabel = getModeLabel;
exports.getOnTheWayLabel = getOnTheWayLabel;
exports.formatEta = formatEta;
const BKK_WH = [13.7367, 100.5231];
const BKK_AIRPORT = [13.69, 100.7501];
const BKK_HOME = [13.752, 100.543];
const DESTINATIONS = {
...
function lerpRoute(points, steps = 8) {
    const out = [];
...
function buildExportRoute(origin, dest, midpoints) {
    return lerpRoute([origin, ...midpoints, dest]);
}
exports.TH_LOCAL_ROUTE = lerpRoute([BKK_WH, [13.738, 100.525], [13.748, 100.539], BKK_HOME]);
const EXPORT_ROUTES = {
...
exports.DELIVERY_ROUTE = exports.TH_LOCAL_ROUTE;
exports.EXPORT_DESTINATION_COUNTRIES = [
...
const SHIPMENT_CATALOG = {
    'TBC-DOM-LOCAL-TH-001': {
        id: 'TBC-DOM-LOCAL-TH-001',
        direction: 'domestic',
        mode: 'local',
        destinationCountry: 'th',
        route: exports.TH_LOCAL_ROUTE,
...
function buildFromSelection(direction, mode, destinationCountry) {
    const found = Object.values(SHIPMENT_CATALOG).find((s) => s.direction === direction &&
        s.mode === mode &&
...
function resolveShipment(trackingId, mode, destinationCountry, direction) {
    let normalized = trackingId.trim().toUpperCase();
    if (LEGACY_ID_MAP[normalized])
        normalized = LEGACY_ID_MAP[normalized];
    if (SHIPMENT_CATALOG[normalized]) {
        return Object.assign({}, SHIPMENT_CATALOG[normalized]);
    }
    const match = normalized.match(/^TBC-(DOM|EXP)-(SEA|AIR|LOCAL)-(TH|CN|JP|GB|RU|US|EU)-/i);
    if (match) {
        const parsedDirection = match[1].toUpperCase() === 'EXP' ? 'export' : 'domestic';
        const parsedMode = match[2].toUpperCase() === 'SEA'
            ? 'sea'
        ...
        const found = Object.values(SHIPMENT_CATALOG).find((s) => s.direction === parsedDirection &&
            s.mode === parsedMode &&
            s.destinationCountry === parsedDest);
        if (found)
            return Object.assign(Object.assign({}, found), { id: normalized });
    }
    if (direction && mode && destinationCountry) {
        const built = buildFromSelection(direction, mode, destinationCountry);
        if (built) {
            return Object.assign(Object.assign({}, built), { id: normalized && normalized !== 'PREVIEW' ? normalized : built.id });
        }
    }
    return null;
}
function getDestinationLabel(country, lang) {
    if (country === 'th')
        return lang === 'th' ? 'ไทย' : 'Thailand';
    const c = exports.EXPORT_DESTINATION_COUNTRIES.find((x) => x.id === country);
    if (!c)
...
function getOnTheWayLabel(mode, direction, lang) {
    if (direction === 'export') {
        if (mode === 'sea')
            return lang === 'th' ? 'กำลังส่งออกทางเรือ' : 'Export in transit (sea)';
        if (mode === 'air')
            return lang === 'th' ? 'กำลังส่งออกทางอากาศ' : 'Export in transit (air)';
    }
    if (mode === 'sea')
        return lang === 'th' ? 'กำลังขนส่งทางเรือ' : 'In transit by sea';
    if (mode === 'air')
        return lang === 'th' ? 'กำลังบินระหว่างทาง' : 'In flight';
    return lang === 'th' ? 'กำลังจัดส่ง' : 'Out for delivery';
}
function formatEta(progress, shipment, lang) {
...
exports.getCountryLabel = getDestinationLabel;
